...
using BiliLive.Kernel.Models.Danmaku;
...
{
    private TaskCompletionSource? _enterRoomStatus;
    public event EventHandler<ReceivedNotificationEventArgs>? ReceivedNotification;
    public event EventHandler<ReceivedHotEventArgs>? ReceivedHot;
    ...
    protected void ReceivedPack(in BiliLivePackHeader header, ReadOnlySpan<byte> body)
    {
        switch (header.Operation)
        {
            case BiliLiveOperation.HeartbeatReply:
                OnReceivedHot(IPAddress.NetworkToHostOrder(BitConverter.ToInt32(body)));
                break;
            ...
            case BiliLiveOperation.EnterRoomReply:
                ...
                if (code is 0)
                {
                    logger.LogInformation("进入房间成功: {code}", code);
                    _enterRoomStatus?.SetResult();
                }
                else
                {
                    logger.LogError("进入房间失败: {json}", json.GetRawText());
                    _enterRoomStatus?.SetException(new BiliApiException("进入房间失败"));
                }
                break;
            default:
                break;
    ...
    protected virtual void OnReceivedHot(int hot)
    {
        ReceivedHot?.Invoke(this, new(hot));
        logger.LogInformation("人气值: {hot}", hot);
    }

    protected void OnReceivedNotification(JsonElement data)
    {
        ReceivedNotification?.Invoke(this, new(data));
